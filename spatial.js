...
{ 
	...
	*/
	...
	if(node_attr_2_color && d3.select("#colorbar_container_"+map_id).empty())
	{
		...
		maps[map_id]["colorbar"] = colorbar;
		
		pointer = d3.selectAll("#colorbar_" + map_id).call(colorbar)
	}
	else 
	if(node_attr_2_color)
		pointer = d3.selectAll("#colorbar_" + map_id).call(maps[map_id]["colorbar"]);
	
	// traverse nodes and set respective map markers style
	d3.json(map_json, function(map_nodes){
	//alert(d3.selectAll("[node-group="+map_id+"]").data());
	
	//var map_nodes = d3.selectAll("[node-group="+map_id+"]").data();
	//alert();
	//var map_nodes = d3.selectAll("[node-group="+map_id+"]").each(function(d){ 
	...
	map_nodes.forEach(function(d){	
			...
			.attr("title", node["NodeLabel"])
			//.attr("title", d.node["NodeLabel"])
			/*
					...
					if(node.hasOwnProperty(node_event_img))
					{
						...
						{	
							...
							if(event_node_image.empty()) // if event node image for this event node has not been added, add event node image
							{
								...
								// ttl will be set to the maximum value; currently hard coded; might want to expose as a parameter
